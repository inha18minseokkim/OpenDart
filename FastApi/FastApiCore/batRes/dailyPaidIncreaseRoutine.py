...
'''
dbClientUrlPaidIncrease = dbUrl + '/tb_paid_increase/'
...
async def getPaidIncreaseEvent():
    # db에서 상장주식 목록 가져온다.
    ...
    targetCorpCode = dbClientUrlCode + f'selectCorpInfoByDay/{sidx}/{eidx}'
    ...
    for i in corpInfoByRangeRes: #후에 로직 수정 후 바꿀 예정
        ...
        if res.json()['status'] != '000':
            if res.json()['status'] == '013':
                logger.debug('dailyPaidIncreaseRoutine 아무것도 없음')
                requestCnt += 1
                continue #정상 응답이 아니면 그냥 continue
            else:
                logger.debug('dailyPaidIncreaseRoutine 뭔가 조회 실패')
                errorCnt += 1
                continue
        for resj in res.json()['list']:
            requestCnt += 1
            try:
                #resj = res.json()['list'][0]
                logger.debug(resj)
                dbres = requests.post(dbClientUrlPaidIncrease + 'setCorpInfo', json=resj,verify=False).json()
                logger.debug(dbres)
                eventCnt += 1
            except:
                logger.debug('dailyPaidIncreaseRoutine '+resj['corp_code'] + ' db 반영 호출 실패')
                errorCnt+=1
            else:
                logger.debug(f"dailyPaidIncreaseRoutine: DB반영 결과 : {dbres['code']}")

    logger.debug(f"dailyPaidIncreaseRoutine 배치 실행 결과 : 총 {requestCnt} 건 이벤트 {eventCnt} 건 실패 {errorCnt} 건")
    return

if __name__ == "__main__":
    getPaidIncreaseEvent()